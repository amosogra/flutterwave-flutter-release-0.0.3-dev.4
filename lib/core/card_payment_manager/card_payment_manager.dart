...
class CardPaymentManager {
  ...
  /// it returns a map
  Map<String, String> _prepareRequest(final ChargeCardRequest chargeCardRequest) {
    final String encryptedChargeRequest = FlutterwaveUtils.tripleDESEncrypt(jsonEncode(chargeCardRequest.toJson()), encryptionKey);
  ...
  Future<dynamic> payWithCard(final http.Client client, final ChargeCardRequest chargeCardRequest) async {
    ...
    try {
      ...
      final url = FlutterwaveURLS.getBaseUrl(this.isDebugMode) + FlutterwaveURLS.CHARGE_CARD_URL;

      final headers = {HttpHeaders.authorizationHeader: this.publicKey, HttpHeaders.contentTypeHeader: "application/json"};
      var payload = {'url': url, 'headers': headers, 'body': encryptedPayload};
      http.Response response;
      if (!kIsWeb) {
        response = await client.post(Uri.parse(url), headers: headers, body: jsonEncode(encryptedPayload));
      } else {
      ...
      if (response.statusCode < 200 || response.statusCode >= 400) {
        throw Exception('Error executing CardPayment transaction: ${response.statusCode} - ${response.reasonPhrase}');
      }
  ...
  void _handleResponse(final http.Response response) {
    try {
      ...
      if (response.statusCode == 200) {
        MetricManager.logMetric(http.Client(), this.publicKey, MetricManager.INITIATE_CARD_CHARGE, "${_stopwatch.elapsedMilliseconds}ms");
        _stopwatch.reset();

        final bool requiresExtraAuth = (responseBody.message == FlutterwaveConstants.REQUIRES_AUTH) && (responseBody.meta?.authorization?.mode != null);

        final bool is3DS = (responseBody.message == FlutterwaveConstants.CHARGE_INITIATED) && (responseBody.meta?.authorization?.mode == Authorization.REDIRECT);

        final bool requiresOtp = (responseBody.message == FlutterwaveConstants.CHARGE_INITIATED) && (responseBody.meta?.authorization?.mode == Authorization.OTP);

        if (requiresExtraAuth) {
          return this._handleExtraCardAuth(responseBody, this.cardPaymentListener);
        }
        if (is3DS) {
          final redirectUrl = responseBody.meta?.authorization?.redirect;
          if (redirectUrl != null) {
            return this.cardPaymentListener?.onRedirect(responseBody, redirectUrl);
          }
          return this.cardPaymentListener?.onError("Unable to complete payment. Please try another card");
        }
        if (requiresOtp) {
          return this.cardPaymentListener?.onRequireOTP(responseBody, responseBody.data!.processorResponse!);
        }

        if (responseBody.status == FlutterwaveConstants.SUCCESS &&
            responseBody.data != null &&
            (responseBody.data!.status == FlutterwaveConstants.SUCCESSFUL || responseBody.data!.status == FlutterwaveConstants.SUCCESS) &&
      ...
      MetricManager.logMetric(http.Client(), this.publicKey, MetricManager.INITIATE_CARD_CHARGE_ERROR, "${_stopwatch.elapsedMilliseconds}ms");
      _stopwatch.reset();

      if (response.statusCode.toString().substring(0, 1) == "4") {
        return this.cardPaymentListener?.onError(responseBody.message!);
      }
      return this.cardPaymentListener?.onError(jsonDecode(response.body).toString());
    } catch (e) {
  ...
  void _handleExtraCardAuth(ChargeResponse response, CardPaymentListener? listener) {
    ...
    if (Authorization.REDIRECT == authMode) {
      ...
      return this.cardPaymentListener?.onError("Unable to complete card charge");
    }
    ...
    if (Authorization.PIN == authMode) return this.cardPaymentListener?.onRequirePin(response);
    return;
  }
